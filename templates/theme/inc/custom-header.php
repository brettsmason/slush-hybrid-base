...
add_action( 'after_setup_theme', '<%= themePrefix %>_custom_header_setup', 15 );
 ...
 * @return void
 */
function <%= themePrefix %>_custom_header_setup() {
	...
	add_theme_support(
		...
		array(
			...
			'header-text'            => true,
			'uploads'                => true,
			'wp-head-callback'       => '<%= themePrefix %>_custom_header_wp_head'
		)
	);
 ...
 * @return void
 */
function <%= themePrefix %>_custom_header_wp_head() {
