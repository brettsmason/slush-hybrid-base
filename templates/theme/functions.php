 ...
 * @link       http://themehybrid.com/themes/<%= themeSlug %>
 * @license    http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
 */

// Get the template directory and make sure it has a trailing slash.
$<%= themePrefix %>_dir = trailingslashit( get_template_directory() );

// Load the Hybrid Core framework and theme files.
require_once( $<%= themePrefix %>_dir . 'library/hybrid.php'        );
require_once( $<%= themePrefix %>_dir . 'inc/custom-background.php' );
require_once( $<%= themePrefix %>_dir . 'inc/custom-header.php'     );
require_once( $<%= themePrefix %>_dir . 'inc/theme.php'             );

// Launch the Hybrid Core framework.
new Hybrid();

// Do theme setup on the 'after_setup_theme' hook.
add_action( 'after_setup_theme', '<%= themePrefix %>_theme_setup', 5 );
 ...
 * @return void
 */
function <%= themePrefix %>_theme_setup() {
